...
def compression(s,now,c):
    ...
    pw=max(x['high'] for x in prior)-min(x['low'] for x in prior)
...
def observe(s,e,c):
    ...
    d=e.data;now=e.received;s['reason']='NO_ELIGIBLE_SETUP'
    ...
    if e.kind in ('QUOTE','DEPTH'):
        ...
        s[key]=dict(d,at=e.at,received=now);return
    if e.kind=='OI':
    ...
    if e.kind=='BAR':
        ...
        if ep is None:s['episode']=compression(s,now,c)
        return
    ...
    meta=s['meta']
    if not ep or not meta or s['warm_trades']<20:return
